 ...
 *
 * agents/*.md files are prompt specs, not executable code, so there is no
 * runtime `route()` function to unit test directly. The closest useful
 * equivalent: cross-check the two places routing IDs are declared
 * (agents/orchestrator.md's "Agent Routing" table and AGENTS.md's "Runtime
 * Subagent Roster" table) against each other and against the actual files
 * in agents/. This is exactly the kind of drift AGENTS.md's own naming note
 * warns about (retired taxonomy names like `sisyphus`/`metis`/`momus`).
 */
...
describe("routing consistency: agents/orchestrator.md routing table", () => {
  ...
  for (const id of Array.from(routingTableIds)) {
    test(`"${id}" has a matching agents/${id}.md file`, () => {
...
describe("routing consistency: no orphan agent files", () => {
  for (const id of agentFiles) {
    test(`agents/${id}.md is referenced in orchestrator.md's routing table`, () => {
      expect(routingTableIds.has(id)).toBe(true);
    });

    test(`agents/${id}.md is referenced in AGENTS.md's roster`, () => {
